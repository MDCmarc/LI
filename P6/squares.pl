...
nonoverlapping( [Side|Sides], [Row|RowVars], [Col|ColVars]):-
        checkAll(Side, Row, Col, Sides, RowVars, ColVars),
        nonoverlapping( Sides, RowVars, ColVars).
     
checkAll(_, _, _, [], [], []).
checkAll(Side, Row, Col, [SideNext|Sides], [RowNext|RowVars], [ColNext|ColVars]):-
        constraints(Side, Row, Col, SideNext, RowNext, ColNext),
        checkAll(Side, Row, Col, Sides, RowVars, ColVars).
       
constraints(Side, Row, Col, SideNext, RowNext, ColNext):-
        Row + Side #=< RowNext;
        Col + Side #=< ColNext;
        RowNext + SideNext #=< Row;
        ColNext + SideNext #=< Col.
