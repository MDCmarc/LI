...
% enforces double(T, R) <--> (home(T, R-1) && home(T, R)) || (-home(T, R-1) && -home(T, R))
coherentDoubles:-
    team(T),
    round(R),
    R \= 1,
    R1 is R - 1,
    writeClause([-home(T, R1), -home(T, R), double(T, R)]),
    writeClause([home(T, R1), home(T, R), double(T, R)]),
    writeClause([-home(T, R1), home(T, R), -double(T, R)]),
    writeClause([home(T, R1), -home(T, R), -double(T, R)]), fail.
coherentDoubles.
...
atmostOneTVMatchPerRound:- 
    round(R),
    tvTeams(TV),
    findall(match(S, T, R), (member(S,TV), member(T,TV), S \= T), Lits),
    atMost(1,Lits), fail.
atmostOneTVMatchPerRound.
