...
eachTaskStartsOnce(Max):- task(T), duration(T,D), findall(start(T,H), (P is Max-D+1, between(1,P,H)),Lits), exactly(1,Lits),fail.
...
setActive(Max):-
    task(T), duration(T,D), 
    P is Max-D+1, between(1,P,Hinici), % get all the posible start hours
    Hfin is Hinici+D-1, between(Hinici,Hfin,Hactive), % get all posible active hours depending on the init and finish hout
    writeClause([-start(T,Hinici), active(T,Hactive)]),
    fail.
setActive(_).
...
resources(infinite):-!.
resources(Max):-
    resourceUnits(R,N), between(1,Max,H),
    findall(active(T,H), usesResource(T,R),Lits), atMost(N,Lits), fail.
resources(_).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
