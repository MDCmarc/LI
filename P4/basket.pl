...
definedouble:-
    round(R1), round(R2), R2 is R1 + 1 , team(T), 
...
noRepeatMatch:-
    ...
    findall(match(T,S,R), round(R), LitsB),
    append(LitsA,LitsB,Lits),
    exactly(1,Lits), fail.
noRepeatMatch.

noDoublesOnRounds:-
    round(R), noDoubles(T), member(R, T), team(S), writeClause([-double(S,R)]), fail.
noDoublesOnRounds.

notHomes:-
    team(T), away(T,R), writeClause([-home(T,R)]), fail. 
notHomes.

noTriples:-
    round(R1), round(R2), round(R3), R2 is R1 + 1, R3 is R1 + 2,  team(T), 
    % REVISAR CLASUSULAS
    writeClause([home(T,R1),home(T,R2),home(T,R3)]),
    writeClause([-home(T,R1),-home(T,R2),-home(T,R3)]), 
    fail.
noTriples.
