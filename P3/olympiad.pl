...
%-----------------------------------------------------------
%AUX
mutfriends(X,Y):- friends(X,Y),friends(Y,X).

sameTheme(X,Y):- expertise(X,E1),expertise(Y,E2), E1 == E2.
%-----------------------------------------------------------

init:- numMembersTeam(N), findall(chosen(S),student(S),Lits), exactly(N,Lits), fail.
init.

mutualfriends:- student(X),student(Y), X \= Y, not(mutfriends(X,Y)), atMost(1,[chosen(X),chosen(Y)]), fail.
mutualfriends.

notmutualthemes:- student(X),student(Y), X \= Y, sameTheme(X,Y), atMost(1,[chosen(X),chosen(Y)]), fail.
notmutualthemes.
