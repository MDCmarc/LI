...
init:- 
    maxStations(K), findall(install(C),city(C),Lits), exactly(K,Lits), fail.
init.

conected:-
    city(X),city(Y),X\=Y, road(X,Y), writeClause( [install(X) , install(Y)] ), fail.
conected.
...
displaySol(M):- write('Team: '), nl, city(X), member(install(X), M), write(X), nl, fail.
displaySol(_):- nl.
