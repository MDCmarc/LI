...
firstAndLastCity:- writeClause([visited(1,0)]),writeClause([visited(1,20)]).
firstAndLastCity.

costMax:- maxCost(M), findall(cost(P), position(P), Lits), atMost(M,Lits).
costMax.

eachCityOnly1:- position(P), findall(visited(C,P), city(C), Lits), exactly(1,Lits), fail. 
eachCityOnly1.

eachPosOnly1:- city(C), C \= 1, findall(visited(C,P), position(P), Lits), exactly(1,Lits), fail.
eachPosOnly1.


getAdjacency:- city(C2), adjacency(C1,L), not(member(C2,L)), position(P1), position(P2), P1 is P2 - 1,
               atMost(1,[visited(C1,P1),visited(C2,P2)]), fail.
getAdjacency.

costRoad:- position(P1), position(P2), P1 is P2 - 1, city(C1), city(C2), K is (C1 + C2) mod 2,
           writeClause([ -visited(C1,P1), -visited(C2,P2), cost(K)]), fail.
costRoad.
