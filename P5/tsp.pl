...
%alternative:
tsp( Cities, AccumulatedKm, Route ):- bestRouteSoFar(Km,_),
    lowerBoundOfRemainingCities( Cities, LBound, Route ),   %implement this (efficiently)!
...
lowerBoundOfRemainingCities( _, LBound, [ CurrentCity | _ ] ):-
    min_dist(CurrentCity,LBound).

myselect(CurrentCity,City,Cities,RemainingCities):-
    min_dist(CurrentCity,K) , distance(CurrentCity,City,K), select( City, Cities, RemainingCities ).

myselect(CurrentCity,City,Cities,RemainingCities):-
    matriu(M), nth1(CurrentCity,M,Row), sort( Row, [_,_|Sorted]),
    select(Dist, Sorted, _),
    distance(CurrentCity,City,Dist),
    select( City, Cities, RemainingCities ).
   


matriu([[   0,144,114,105, 31,109,135,132, 85, 79,158, 20, 73,162,127,190,156, 58, 87, 71,154, 55],
