...
eachCourseExactlyOneProf:- 
    courseProfessors(C,LP), findall( cp(C,P), member(P,LP), Lits ), exactly(1,Lits), fail.
eachCourseExactlyOneProf.
    

eachCourseExactlyOneRoom:-
    courseRooms(C,Rooms), findall(cr(C,R), member(R,Rooms),Lits), exactly(1,Lits), fail.
eachCourseExactlyOneRoom.


eachCourseMaxOncePerDay:-
    course(C), day(D), findall(cdh(C,D,H), hour(H),Lits), atMost(1,Lits), fail.
eachCourseMaxOncePerDay.


relateVarsCDHwithCD:-      
	course(C), day(D), findall(cdh(C,D,H), hour(H), Lits), expressOr(cd(C,D),Lits), fail.
relateVarsCDHwithCD.


eachCourseRightNumberOfDays:-
	course(C), courseHours(C,H), findall(cd(C,D),day(D),Lits),  exactly(H,Lits), fail.
eachCourseRightNumberOfDays.


noOverlapCoursesSameYear:- 
    courseYear(C1,Y), courseYear(C2,Y), C1\=C2, day(D), hour(H), writeClause([ -cdh(C1,D,H), -cdh(C2,D,H)]), fail. 
noOverlapCoursesSameYear.


noOverlapProfs:-
	courseProfessors(C1,P1), courseProfessors(C2,P2), C1 \= C2 , member(P,P1), member(P,P2), day(D), hour(H),
	writeClause([ -cdh(C1,D,H), -cdh(C2,D,H), -cp(C1,P), -cp(C2,P) ]), fail. 
noOverlapProfs.


noOverlapRooms:-
	courseRooms(C1,R1), courseRooms(C2,R2), C1\=C2, member(R,R1), member(R,R2), day(D), hour(H),
	writeClause([ -cdh(C1,D,H), -cdh(C2,D,H), -cr(C1,R), -cr(C2,R) ]), fail. 
noOverlapRooms.


relateVarsCPwithUsedProf:-
	courseProfessors(C,Plist), member(P,Plist), writeClause([-cp(C,P), usedProf(P)]), fail.
relateVarsCPwithUsedProf.

maxProfs(MaxNumProf):-
	findall(usedProf(P), professor(P),Lits), atMost(MaxNumProf,Lits), fail.
maxProfs(_).
